function (Y, X, V = 20, family = gaussian(), SL.library, method = "method.NNLS", 
          id = NULL, verbose = FALSE, control = list(saveFitLibrary = FALSE), 
          cvControl = list(), obsWeights = NULL, saveAll = TRUE, parallel = "seq") 
{
  call <- match.call()
  N <- dim(X)[1L]
  ...
  SL.predict[unlist(folds, use.names = FALSE)] <- unlist(lapply(cvList, 
                                                                "[[", "cvSL.predict"), use.names = FALSE)
  discreteSL.predict[unlist(folds, use.names = FALSE)] <- unlist(lapply(cvList, 
                                                                        "[[", "cvdiscreteSL.predict"), use.names = FALSE)
  whichDiscreteSL <- lapply(cvList, "[[", "cvwhichDiscreteSL")
  library.predict[unlist(folds, use.names = FALSE), ] <- do.call("rbind", 
                                                                 lapply(cvList, "[[", "cvlibrary.predict"))
